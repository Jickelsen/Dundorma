...
FROM ubuntu:16.04
MAINTAINER Jacob Michelsen <chromegadget@gmail.com>

RUN apt-get update && \
    apt-get -y install apache2 php libapache2-mod-php php-mcrypt php-json curl php-curl git nodejs npm php-mysql unzip php-mbstring php-dom vim && \
    apt-get clean && \
...
# Update php config
RUN echo "extension=php_mbstring.dll" >> /etc/php/7.0/apache2/php.ini && echo "extension=php_curl.dll" >> /etc/php/7.0/apache2/php.ini 
